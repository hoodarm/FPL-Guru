...
for item in players_statistics:
        if item["Name"] in positionWisePlayerData["GK"]:
            item["Form rating"] = round(0.1 * float(item["Saves"]) + 0.7 * float(item["Clean sheets"]) - 0.2 * float(item["Goals conceded"]) + 0.1 * float(item["Penalties saved"]) + 0.1 * float(item["Total points"]) + 0.2 * float(item["Percentage selection"]),1)
            item ["Position"] = "GK"

for item in players_statistics:
        if item["Name"] in positionWisePlayerData["DF"]:
            item["Form rating"] = round(0.3 * float(item["Clean sheets"]) + 0.1 * float(item["Starts"]) + 0.2 * float(item["Goals"]) + 0.1 * float(item["Assists"]) + 0.1 * float(item["Total points"]) + 0.2 * float(item["Percentage selection"]),1)
            item ["Position"] = "DF"
            
for item in players_statistics:
        if item["Name"] in positionWisePlayerData["FW"]:
            item["Form rating"] = round(0.55 * float(item["Goals"]) + 0.1 * float(item["Assists"]) + 0.05 * float(item["xG"]) + 0.1 * float(item["Total points"]) + 0.2 * float(item["Percentage selection"]),1)
            item ["Position"] = "FW"
...
class MyTeamPage_Pitch(tk.Frame):
    ...
    def get_position_weights(self):
        ...
        try:
            ...
            return {"GK": 2, "DF": 4, "MF": 3, "FW": 1}

        # Validate that the number of weights matches the number of positions
        if len(weights) == 4 and weights[0] + weights[1] + weights[2] + weights[3] == 10:
            return {"GK": weights[0], "DF": weights[1], "MF": weights[2], "FW": weights[3]}
        else:
            # Handle invalid input (wrong number of weights)
            tk.messagebox.showerror("Error", "Invalid input. Please enter exactly 4 positional weights summing up to 10.")
            return {"GK": 2, "DF": 4, "MF": 3, "FW": 1}
    ...
    def display_team_on_pitch(self, team):
        ...
        # Load and resize the background image for the pitch
        self.background_image = Image.open("GUI/Canvas.jpg")
        self.background_image = self.background_image.resize((900, 540), Image.ANTIALIAS)
        self.background_image = ImageTk.PhotoImage(self.background_image)
